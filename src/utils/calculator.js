...
const EUMDOL = {
  ...
  2: { speedRate: 0.1, exceedBonus: 8, excessRate: 0.3, maxTotal: 24 },
};
...
function calcNodeDPS(
  ...
  const critFactor =
    eCR * (critDmg / 100) * (1 + critEnemyDmg / 100) + (1 - eCR);
...
export function calculate(state) {
  ...
  const necklaceAdditionalDmg =
    findOption(NECKLACE_OPTIONS, state.necklaceOption).additionalDmg || 0;

  const ring1CR =
    findOption(RING_CRIT_RATE_OPTIONS, state.ring1CritRate).critRate || 0;
  const ring1CD =
    findOption(RING_CRIT_DMG_OPTIONS, state.ring1CritDmg).critDmg || 0;
  const ring2CR =
    findOption(RING_CRIT_RATE_OPTIONS, state.ring2CritRate).critRate || 0;
  const ring2CD =
    findOption(RING_CRIT_DMG_OPTIONS, state.ring2CritDmg).critDmg || 0;
  ...
  const rcConversion = rcActive
    ? RC_CONVERSION[state.raidCaptainGrade] +
      STONE_BONUS[state.raidCaptainStone]
    : 0;

  const kbwActive = state.keenBluntGrade !== 'none';
  const kbwCritDmg = kbwActive
    ? KBW_CRIT_DMG[state.keenBluntGrade] + STONE_BONUS[state.keenBluntStone]
    : 0;

  const adrCritRate = ADRENALINE_CRIT[state.adrenalineGrade] || 0;

  const evoCritRate =
    state.evo2KeenSense * 4 +
  ...
  const staticEvoDmg =
    supporterEvoDmg +
    state.evo2ForbiddenSpell * 10 +
    state.evo2LimitBreak * 10 +
    state.evo2KeenSense * 5 +
    state.evo2OptTraining * 5 +
    state.evo3InfinitePower * 8 +
    state.evo3TimingDomination * 8 +
    state.evo3AllOutStrike * 2 +
    state.evo3DestructionTank * 12 +
    state.evo5Infighting * 9 +
    state.evo5StandStriker * 10.5 +
  ...
  const raidCaptainEfficiency = rcActive
    ? floor2((moveSpeedExcess * rcConversion) / 100)
    : 0;
  ...
  if (kbwActive) {
    ...
    kbwEfficiency =
      dpsWithout > 0 ? floor2((dpsWith / dpsWithout - 1) * 100) : 0;
  }
  ...
  const eumdolEvoDmg = floor2(
    calcEumdolEvoDmg(rawAtkSpeed, rawMoveSpeed, state.evo5Eumdol),
  );
  ...
  if (state.evo5Eumdol > 0) {
    ...
    eumdolActivationSwift = Math.max(
      0,
      Math.ceil(Math.max(needAtk, needMove) + 0.01),
    );
    ...
    if (remaining > 0 && e.excessRate > 0) {
      ...
      eumdolFullEffSwift = Math.max(
        eumdolActivationSwift,
        Math.ceil(neededSpeedFromSwift * 58.22),
      );
    } else {
  ...
  const additionalDmgNo4T =
    weaponAdditionalDmg +
    necklaceAdditionalDmg +
    braceletAdditionalDmg +
    grid.additionalDmg;
  const critEnemyDmgNo4T = braceletCritEnemyDmg + grid.critEnemyDmg;
  ...
  const bestCombo = node4Combos.reduce((best, c) =>
    c.multiplier > best.multiplier ? c : best,
  );
